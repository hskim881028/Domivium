...
## 1. Project Overview
- Unity 6.2 버전을 사용해서 게임 개발.
- pc, iOS, android, 닌텐도 스위치에 출시예정.
- DB는 PostgreSQL 사용
- 서버는 MagicOnion패키지를 활용해서 구성
- 클라이언트는 VContainer패키지를 활용해서 구조 구성
- UI는 MVP 구조에 VM을 섞어서 구성.
- Escape from Duckov게임과 동일한 장르의 게임을 2d 모바일 게임으로 제작.
- x,y 타일맵 top view로 진행.
- 게임 전투 시스템은 Unreal Engine에 있는 Gameplay Ability System와 동일하게 구성.
- BattleSystem이 GameplaySystemComponent와 비슷
